...
<div class="chat-log" :class="theme">
    <p v-if="theme === 'terminal'" class="head">/Documents/Room/{{roomName}}</p>
    <chat-message v-for="message in messages" :message="message" :key="message.id" :class="{'me': isMe(message.user.name), 'not-me' : !isMe(message.user.name)}">
...
export default {
    ...
    methods: {
        isMe(user){
            return user.trim() == $('.navbar-right .dropdown-toggle').text().trim();
        }
    },
